...
import ColoredLineProgram from '../webgl/program/ColoredLineProgram'
import ColoredPointProgram from '../webgl/program/ColoredPointProgram'
import Face2DProgram from '../webgl/program/Face2DProgram'
import Line2DProgram from '../webgl/program/Line2DProgram'
import PickProgram from '../webgl/program/PickProgram'
import TexturedFaceProgram from '../webgl/program/TexturedFaceProgram'
...
export default class WebglDevice implements Device {
  constructor(canvas: HTMLCanvasElement, eventHandler: EventHandler) {
    ...
    this.coloredPointProgram = new ColoredPointProgram(
      webgl,
      this.createArrayTexture(
        ColoredPointProgram.pointTextureImage,
        64,
        64,
      ) as WebglTexture,
    )
  ...
  drawPoint(context: Context, buffer: PointBuffer, color: Color): void {
    ...
    webgl.bindBuffer(
      webgl.ARRAY_BUFFER,
      (buffer.vertexes as WebglVertexBuffer).buffer,
    )

    webgl.vertexAttribPointer(
      pointProgram.vertexAttribPosition,
      3,
      webgl.FLOAT,
      false,
      0,
  ...
  drawLine(
    ...
    const lineProgram = isDashedLine
      ? this.coloredDashedLineProgram
      : this.coloredLineProgram
    ...
    webgl.vertexAttribPointer(
      lineProgram.vertexAttribPosition,
      3,
      webgl.FLOAT,
      false,
      0,
    ...
    if (isDashedLine) {
      ...
      webgl.bindBuffer(
        webgl.ARRAY_BUFFER,
        (buffer.lengths as WebglVertexBuffer).buffer,
      )
      webgl.vertexAttribPointer(
        dashedLineProgram.lengthAttribPosition,
        1,
        webgl.FLOAT,
        false,
        0,
  ...
  drawLine2D(context: Context, buffer: LineBuffer, color: Color): void {
    ...
    webgl.vertexAttribPointer(
      line2DProgram.vertexAttribPosition,
      2,
      webgl.FLOAT,
      false,
      0,
  ...
  drawFace(
    ...
    const webgl = this.webgl
    const faceProgram = texture
      ? this.texturedFaceProgram
      : this.coloredFaceProgram
    ...
    webgl.vertexAttribPointer(
      faceProgram.vertexAttribPosition,
      3,
      webgl.FLOAT,
      false,
      0,
    ...
    webgl.bindBuffer(
      webgl.ARRAY_BUFFER,
      (buffer.normals as WebglVertexBuffer).buffer,
    )
    webgl.vertexAttribPointer(
      faceProgram.normalAttribPosition,
      3,
      webgl.FLOAT,
      false,
      0,
    ...
    if (faceProgram instanceof TexturedFaceProgram) {
      ...
      webgl.bindBuffer(
        webgl.ARRAY_BUFFER,
        (textureBuffer.coordinate as WebglVertexBuffer).buffer,
      )
      webgl.vertexAttribPointer(
        program.textureCoordPosition,
        2,
        webgl.FLOAT,
        false,
        0,
      ...
      webgl.bindTexture(
        webgl.TEXTURE_2D,
        (textureBuffer.texture as WebglTexture).texture,
      )
      webgl.uniform1i(
        program.samplerUniformPosition,
        (textureBuffer.texture as WebglTexture).number -
          WebglTexture.StartIndex,
      )
    }
    ...
    if (buffer.indexes) {
      webgl.bindBuffer(
        webgl.ELEMENT_ARRAY_BUFFER,
        (buffer.indexes as WebglIndexBuffer).buffer,
      )
  ...
  drawFace2D(context: Context, buffer: FaceBuffer, color: Color): void {
    ...
    webgl.vertexAttribPointer(
      face2DProgram.vertexAttribPosition,
      2,
      webgl.FLOAT,
      false,
      0,
  ...
  pick(
    ...
    webgl.vertexAttribPointer(
      pickProgram.vertexAttribPosition,
      3,
      webgl.FLOAT,
      false,
      0,
